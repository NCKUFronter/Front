  ...
  <v-content>
    <v-container fluid style="width: 960px">
      <v-flex xs12 sm12 md12 class="text-center">
        <v-img
          :src="personal.img"
          style="border-radius: 50%; height:200px; width:200px; margin: auto; margin-top: 20px;"
        ></v-img>
        <v-card flat v-if="personalModal">
          ...
          <v-icon small class="edit" @click="editPersonal()">mdi-pencil</v-icon>
        </v-card>
        <v-card flat v-else class="editModal">
          <v-text-field label="使用者名稱" v-model="personal.username" class="editContent"></v-text-field>
          <v-text-field label="使用者ID" v-model="personal.ID" class="editContent"></v-text-field>
          <v-icon small class="edit" @click="editPersonal()">mdi-content-save-move-outline</v-icon>
        </v-card>
