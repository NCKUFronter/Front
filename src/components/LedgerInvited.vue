<template>
    ...
    <v-flex xs6 sm3 md3 v-for="invitation in invitations" :key="'invite'+invitation._id" class="pa-2">
      ...
      <v-card-text class="pa-0 ma-0" style="font-size:10px">
          {{ invitation.fromUser.name }}邀請了你
      </v-card-text>
        <v-spacer />
        <v-btn  class="ma-0 px-1 pa-0 elevation-0" style="font-size:10px;height:fit-content;width:fit-content;border-radius:0;" v-on:click="answerInvitation(invitation._id, true)">加入</v-btn>
        <v-btn class="ma-0 px-1 pa-0 elevation-0" style="font-size:10px;height:fit-content;width:fit-content;border-left:1px solid white;border-radius:0;" v-on:click="answerInvitation(invitation._id, false)">拒絕</v-btn>
      </v-card-actions>
